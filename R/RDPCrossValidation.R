...
RDPCrossValidation <- function(spikes, t.start = 0, t.end,
  ...
  count.points <- lapply(1:length(N.values), function(J) {
    lapply(
      1:n.trials, function(i) {
  ...
  if (PSTH) {
    ...
    # Helper function that computes ISE for a (J, lambda.idx) pair
    compute.ISE <- function(J, lambda.idx) {
      val <- N.values[J]
      lambda <- poss.lambda[lambda.idx]
      ...
      f.hat.minus.i <- matrix(f.hat * (n.trials / (n.trials - 1)),
        nrow = n.trials, ncol = val, byrow = TRUE
      ) - f.hat.i / (n.trials - 1)
      ...
      f.hat.minus.i.bar <- vapply(1:n.trials, function(i) {
        if (length(spikes[[i]]) > 0) {
          ...
          f.hat.minus.i.ti <-
            sum(til == points[1]) * f.hat.minus.i[i, 1] +
            sum(til == points[length(points)]) * f.hat.minus.i[i, ncol(f.hat.minus.i)] +
  ...
  min.ISE.for.each.J <- c(ISE.for.J.equal.to.0, apply(
    ISE.matrix,
    2, min
  ))
  ...
  if (min.J.allowable == 0) {
    ...
    cv.output.list <- list(
      J.ISE = min.J.allowable, lambda.ISE = poss.lambda[lambda.min.ISE.index],
      ISE = ISE.matrix
    )
  }
  if (pct.diff.plot) {
