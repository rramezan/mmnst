...
```{r, fig.align='center', fig.width=5 ,echo=TRUE}
...
#### Step 1: setting the start and the end time of the observation window
...
#### Step 2: Find the peak frequencies in each spike train
...
#### Step 3: perform the actual cross-validation
...
cv.output <- RDPCrossValidation(spikes, t.start , t.end , poss.lambda, max.J, PSTH=FALSE, max.diff = 0.005 , pct.diff.plot = TRUE, allow.parallel = TRUE)
...
#### Step 4: Get the stepwise intensity function c(t)
```{r, echo=TRUE}
...
#### Step 5: Identify the terminal points (boundaries at which c(t) changes)
```{r, echo=TRUE}
...
#### Step 6: Get the setup variables needed for the likelihoods
```{r, echo=TRUE}
...
#### Step 7: Run the model fitting algorithms
...
#### Step 8: Estimate intensity functions of all possible 2K+1 models
...
#### Step 9: Plot the intensity functions of Step 7.
```{r, fig.align='center', fig.width=8 ,echo=TRUE}
#pdf("Outsinc.pdf")
...
#### Step 10: Goodness-of-fit test (pp-plot) for all 2K+1 models
...
#### Step 11A: Estimate intensity functions of best of 2K+1 models by AICc and BIC
...
#### Step 11B: Some statistics from the models which might be useful. We used these in our simulations.
```{r, echo=TRUE}
